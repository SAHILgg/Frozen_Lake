...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while(1):
        ...
        while(state <nS):
            ...
            value_function[state] = P[state][action][0][2] + gamma*P[state][action][0][0]*prev_value_function[P[state][action][0][1]]
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    while(state<nS):
        ...
        max=0
        while(action<nA):
            action_value = P[state][action][0][2] + gamma*P[state][action][0][0]*value_from_policy[P[state][action][0][1]]
            if(action_value > max):
                max = action_value
